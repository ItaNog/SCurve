...
def plot_s_curve(project_name, ylim, 
                 ...
                 delta_end_days=None,  # Add this parameter
    ...
    if baseline_s_curve_cumulative is not None:
        ...
        ax.plot(dates_filtered, baseline_s_curve_filtered,
    ...
    if detailed_s_curve is not None and detailed_dates is not None:
        ...
        ax.plot(detailed_dates_filtered, detailed_s_curve_filtered,
    ...
    if customer_forecast_curve is not None and customer_forecast_today is not None:
        ...
        ax.plot(dates_filtered, customer_forecast_curve_filtered,
    ...
    if actual_progress_curve is not None and actual_dates is not None:
        ...
        ax.plot(actual_dates_filtered, actual_progress_filtered,
    ...
    if workday_s_curve is not None:
        ...
        ax.plot(workday_dates_filtered, workday_s_curve_filtered,
    ...
    if manpower_plan_s_curve is not None and mp_dates_filtered is not None:
        ...
        ax.plot(mp_dates_filtered, manpower_plan_s_curve_filtered, 
                 label=f'Mandays Forecast ({max_hours_percentage:.2f}%)',
                 linestyle='--', color='purple', linewidth=1)

        # Add text for Mandays Overrun (delta_MP)
        ax.text(0.95, 0.01, f'Mandays Overrun: {delta_MP:.2f}%', 
                 verticalalignment='bottom', horizontalalignment='right', 
                 transform=ax.transAxes, color='purple', fontsize=10)

        # Add text for Delta Forecast-Baseline
        if delta_end_days is not None:
            ax.text(0.95, 0.05, f'Delta Forecast-Baseline: {delta_end_days} days', 
                     verticalalignment='bottom', horizontalalignment='right', 
    ...
    ax.set_title(f'Site Progress for Project {project_name}')
    ax.grid(True)
    ax.legend()
...
if __name__ == "__main__":
    ...
    if engine:
        ...
        st.title('Progress Control Dashboard')
        
        # Create a dropdown for project selection
        project_name = st.selectbox('Select a project', project_names)
        
        if project_name:
            st.write(f"Generating S-curve for project: {project_name}")
            ...
            if df_baseline.empty and customer_df.empty and activity_df.empty and detailed_df.empty:
                st.warning(f"No data available for project: {project_name}")
            else:
                #--------------------------------------------
                ...
                if not dates_list:
                    st.warning(f"No date data available for project: {project_name}")
                else:
                    ...
                    final_workday_value = workday_s_curve[-1] if workday_s_curve is not None else 0
                    ...
                    fig = plot_s_curve(
                        ...
                        delta_MP=delta_MP,
                        delta_end_days=delta_end_days,  # Pass delta_end_days
